...
**Goal:** Enable reading and creating work items/PRs across Azure DevOps (`<org>` org — Boards + Repos, multiple projects) and GitHub (`<org>` org, multiple repos), without an MCP server.
...
- Pack (`claude-agent-pack`) must stay generic/portable — no <org>-specific org/project names hardcoded into skill bodies
...
- Org/project/repo values supplied via environment variables, with multi-project support as a **delimited list in one var**:
  ...
  GITHUB_ORG=<org>
  GITHUB_REPOS=<repo-b>,other-repo
  ```
