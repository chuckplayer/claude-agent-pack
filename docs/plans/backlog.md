...
- **The reciprocal key goes into the tracker's own field at creation time, and this binds the batch-write
  cut.** `key: <feature>:<item-id>` (e.g. `vendor-sync:STORY-3`) is written **into the tracker**, not only
...
- **`/backlog` runs once per spec, not once per plan, and its tree covers every `active` requirement.**
  ...
  across files nobody enumerates. The real `vendor-sync` spec — 15 requirements, plainly several cuts —
...
- **A requirement the spec leaves open gets a `SPIKE`, never an implementation story — and the rule
  ...
  `vendor-sync` spec this blocks **7 of 15** requirements, `REQ-011` among them, and that is accepted
...
- **Item ids (`FEATURE-n`, `STORY-n`, `TASK-n.m`, `SPIKE-n`) ship, and they are frozen append-only from
  ...
  is now also the **content** of the reciprocal key written into the tracker (`vendor-sync:STORY-3`), so
...
This is not a nicety. The real `vendor-sync` spec names `REQ-001` mid-sentence inside a bullet labelled
