...
- **The query is the filter; exact equality is the identity test, and the two are different
  ...
  `rbe` also returns an item tagged `vendor-sync:STORY-3` from an unrelated feature. Therefore: the
...
- **NEW, and the substantive one: 8d now requires a positive control before trusting a blank read.**
  ...
  `UNKNOWN` and stopped the run before any write. **Executing BAR-014 against `<org>/<project-a>` proved
...
- **BAR-014.** Original wording, verbatim: *"run the key query and confirm the invocation **succeeded**
  ...
  consequence of the 8d deviation above. Verified against `<org>/<project-a>` on 2026-08-03.
...
**Tenth departure, added after `dd5df72` — the anchor tag. This one corrects a design defect that
shipped, and it is the most substantive entry in this section.**

- **Stated call: the reciprocal key is a single tag `<feature>:<item-id>`, reconciled by one query
  `[System.Tags] CONTAINS '<feature>:'`** -> shipped as **two tags per item** — the key tag
  `<feature>:<item-id>` plus a bare **anchor tag** `<feature>` — reconciled by
  `[System.Tags] CONTAINS '<feature>'`. Decided by: coordinating session, on evidence from a live probe.

  **Why:** `[System.Tags] CONTAINS` is **whole-tag membership, not substring matching.** Verified against
  `<org>/<project-a>` with a real work item carrying `zzprobe-a:STORY-1`: `CONTAINS 'zzprobe-a:'` matched
  **nothing**, `CONTAINS 'zzprobe-a:STORY-1'` matched, and `=` errored as unsupported on the field. So
  the stated query returns **zero rows forever**, on a tracker full of the feature's items.

  **What that would have caused.** Every already-created item falls to row 1's no-entry case and is
  **created again on every run** — the precise duplication the six-row table exists to prevent, promoted
  from edge case to default path. **The positive control added earlier in this section does not catch
  it:** the query is valid and merely semantically wrong, so the control finds the mechanism healthy and
  the table is then handed a confidently wrong empty result. Recorded in
  `memory/known-issues/2026-08-03-wiql-tags-contains-is-whole-tag-not-substring.md`.

  **One prior concern is void as a consequence.** The substring-collision hazard — `rbe` matching
  `vendor-sync:STORY-3` — that `devils-advocate` raised and `security-reviewer` reinforced **cannot
  occur**, because there is no substring matching. Exact client-side equality is retained for a
  different reason, now stated in 8d: the anchor query returns every item of the feature, so exact
  matching is what resolves *which* item a row is.

  **A second defect fixed in the same pass.** The tag round-trip probe read the tag back via a
  shell-quoted JMESPath projection, which returned **empty for a tag that was present and correct** —
  the hazard in `memory/context/2026-07-30-powershell-mangles-native-exe-arguments.md`. A probe using
  that method would have stopped the batch reporting "recovery-by-key unavailable" on a project where
  tagging works. 8f now requires reading `--output json` and searching the object.

**Two further bars amended, under the same four conditions, as named consequences of the departure
above.**

- **BAR-003.** Original wording, verbatim: *"`skills/devops-azure/SKILL.md` section 8f contains the
  literal `System.Tags` as the field, and the value form `<feature>:<item-id>` with an explicit
  statement that **no prefix or namespace is added**."* Amended to require **both** tags — the key tag
  `<feature>:<item-id>` and the anchor tag `<feature>` — written in the same create operation, and to
  require the round-trip probe to confirm both. The no-prefix requirement on the key tag is unchanged.
- **BAR-004.** Original wording, verbatim: *"It contains the WIQL query form with
  `[System.Tags] CONTAINS` and a select list naming …"* and, in its addition (i), *"`[System.Tags]
  CONTAINS '<feature>:'` is a **server-side narrowing filter and never the identity test**"* with the
  substring-collision rationale. Amended so the query matches the **anchor tag** `<feature>`, and so
  addition (i) states the actual semantics (whole-tag membership; a prefix query matches nothing) with
  exact equality justified as row-identity resolution rather than as collision prevention. The
  six dispositions, the never-re-create rule, and additions (ii)–(iv) are unchanged.

Both amendments are narrowly scoped to the clauses the departure falsified. No bar was renumbered, and
no other bar text was touched.
...
- **The end-to-end bar needs a real ADO project, and the permission it needs is not "throwaway".**
  ...
  **RESOLVED 2026-08-03: the gate was met and BAR-015 ran, PASS on all four sub-bars, against
  `<org>/<project-a>`.** Nothing in this cut is specification-only any more — the six-row table, the
  resume path, the repair row, the tag round-trip and the WIQL key query have each been executed
  against a live org. See BAR-015's amendment in `## Acceptance bars` for the item ids and per-sub-bar
  evidence. **Two corrections this paragraph got wrong** and which the run settled: the tag namespace
  is **per-project, not org-wide** (verified — `<project-a>` held one tag, `<project-a>` seventeen, each namespaced
  by project GUID), so "roughly ten permanent org-wide tag values" above overstates the blast radius
  while the **permanence** half of the claim stands exactly as written; and the run cost **13**
  permanent items across two trees rather than roughly ten. The residual risk this paragraph was
  written to flag is discharged, and it is left in place unedited above the resolution because a plan
  is the record of what a change meant to do.
...
Failure behavior: ONE query per run, `[System.Tags] CONTAINS '<feature>:'` in the resolved project,
        ...
        `vendor-sync:STORY-3`. A returned row counts as an item's key ONLY when its tag set holds a
...
- **The identity rule as its own sentence.** `[System.Tags] CONTAINS '<feature>:'` is the **server-side
  ...
  `rbe` also returns `vendor-sync:STORY-3`. A returned row counts as an item's key **only** when its tag
...
- BAR-003: the reciprocal key's ADO field is named, its value carries no prefix, and the file **specifies** a round-trip probe on the first created item with a hard stop on failure. **This is a text check and its subject line says so deliberately** — nothing here runs the probe, and a reader reporting PASS has not verified that a colon survives an ADO tag. The probe's behaviour is exercised only in BAR-015(c), which is gated and may return NOT RUN
  Evidence: files -> `skills/devops-azure/SKILL.md` section 8f contains the literal `System.Tags` as the field, and **two tag values written in the same create operation**: the key tag `<feature>:<item-id>` with an explicit statement that **no prefix or namespace is added**, and the bare **anchor tag** `<feature>` whose stated purpose is making the feature's items findable by a whole-tag query. The round-trip probe must confirm **both**. **Amended 2026-08-03 — the original clause required a single tag; `[System.Tags] CONTAINS` proved to be whole-tag membership, so a single colon-bearing tag is unqueryable by feature. See `## Deviations`.** It states the key is written **in the same operation that creates the item**, with the timing given as load-bearing rather than incidental. The round-trip probe is present: after the **first** item of the run, read that item back and confirm the tag matches **exactly**; on any mismatch, write that item's `external_refs:` entry and **stop the batch**, reporting that recovery-by-key is unavailable in this project. The four rejected alternatives are named with their reasons (custom field, description or comment, hyperlink relation, title prefix). Cross-check the inherited sentence survives: `skills/backlog/SKILL.md` still states that the same value is written into the tracker's own field, and `git diff $BASE -- skills/backlog/SKILL.md` shows no hunk touching it
