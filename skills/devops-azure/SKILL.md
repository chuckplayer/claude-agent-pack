...
**The reason it cannot be cosmetic — restated 2026-08-03, because the reason previously given here was false.** This paragraph used to argue that parent/child validity *is* type-constrained by the process template, so a tree holding a `User Story` parent from run 1 and a `Product Backlog Item` parent from run 2 would **fail the link pass** mid-run. It would not: per 8c above, every type pair is accepted. **The STOP is still right and its justification was backwards.** A mixed-type hierarchy is created **successfully**, and the damage arrives afterwards and silently — a board that will not reorder, and items missing from sprint backlogs — with no failed command, no non-zero exit, and nothing in the run report to notice. **That is strictly worse than the mid-run failure this paragraph used to fear**, because a mid-run failure is at least visible and resumable, while this leaves a tracker that looks correct and behaves wrongly, in a mode with no delete path to undo it.
