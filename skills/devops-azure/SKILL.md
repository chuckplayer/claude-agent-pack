---
name: devops-azure
description: "Read and create Azure DevOps work items and PRs via the az CLI (devops extension) for the org/projects configured in AZURE_DEVOPS_ORG/AZURE_DEVOPS_PROJECTS. Use for one-off Azure Boards/Repos operations outside the code pipeline: viewing or creating work items, viewing PRs, commenting, updating state, linking work items to PRs/commits. Also provides batch write mode (step 8), which consumes a reviewed feature/story/task tree produced by /backlog and creates the whole tree as work items in one pass -- previewing every write, taking one confirmation for the batch, and writing the resulting ids back into the tree. Trigger this when someone says: check my work items, list open PRs in Azure DevOps, create a work item, comment on PR #N, close this work item, link this commit to a work item, connect to azure devops, smoke test the az cli, create the work items from this backlog, push this backlog tree into ADO, bulk create these stories, create the whole tree in Azure Boards. Do NOT use for GitHub PRs/issues -- that's skills/devops-github. Do NOT use for a full code review or implementation pipeline -- use /review-pr or /implement instead. Do NOT use to decompose a spec of record into a backlog -- that is /backlog, which produces a reviewed feature/story/task tree first; batch write mode consumes that tree and never produces one."
---
...
- **`[System.Tags] CONTAINS` narrows, exact equality decides:** `CONTAINS` is a substring match, so a feature slug that is a substring of another (`rbe` inside `profac-rbe`) matches a foreign item. Compare the split tag set for exact equality client-side, or you get silent under-creation.
