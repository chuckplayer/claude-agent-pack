...
**What actually happens, executed against `<org>/<project-a>`:**

- The pair is **created successfully**, exit 0. Confirmed.
- **Only the child becomes unreorderable.** `PATCH _apis/work/workitemsorder` on it returns HTTP 400, `typeKey: SameTypeHierarchyException`, naming the item and its two remedies. **The board is fine** — the parent reorders exit 0, and so does every unrelated item on the same backlog.
- The child is **not hidden** from the product backlog and **not missing** from the sprint backlog. Both were checked with positive controls; both claims failed to reproduce.
- **The trigger is the backlog *category*, not the type.** `Microsoft.RequirementCategory` holds `Product Backlog Item`, `User Story`, **and** `Bug`, so `User Story → Bug` fails identically to `User Story → User Story`. **A rule keyed on "same type" misses that case** — the exception's own name is misleading.

**So the degradation is narrow and self-announcing, not broad and silent**, and the earlier "strictly worse than a mid-run failure" framing overstated it. The STOP earns its place on the narrower ground: a same-category child stays unreorderable until someone re-parents it or converts the link to `Related`, and a resume cannot detect that because the create it came from succeeded. Every result above is the **REST surface** — whether the backlog *page* renders such a child differently is untested and would need a human or a browser.
