...
- **Skill name conflicts with a built-in Claude Code skill:** Before writing, check whether the proposed name matches a built-in Claude Code skill. Known built-in skill names include: `init`, `review`, `security-review`, `simplify`, `diff-review`, `fact-check`, `generate-slides`, `generate-visual-plan`, `generate-web-diagram`, `plan-review`, `project-recap`, `update-config`, `keybindings-help`, `fewer-permission-prompts`, `loop`, `schedule`, `claude-api`, `branch-safety`, `commit`, `explain-code`, `visual-explainer`. If there is a name collision, warn the developer — the pack skill will shadow the built-in. Suggest a disambiguated name (e.g., prefix with the domain: `db-review` instead of `review`, or `pack-init` instead of `init`) unless shadowing is the explicit intent.
