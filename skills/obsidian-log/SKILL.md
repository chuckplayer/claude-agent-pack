...
**Attempt the PUT** using the same PowerShell-first, curl-fallback pattern from obsidian-capture step 3a. Substitute `$vaultRel` with the path computed above and `$body` with the session markdown content. The result sets `$apiWritten` / `apiWritten` to `true` or `false`.
...
> "Session logged to `<org>/agent-pack/sessions/2026-05-14-1430-agent-pack.md` (via REST API)"

or:

> "Session logged to `<org>/agent-pack/sessions/2026-05-14-1430-agent-pack.md` (filesystem)"
