...
as large as it is. Tested against the real 15-requirement `profac-rbe` spec: the blocking rule blocks
...
**Unresolved, known risk.** On `profac-rbe`: one tree, written at cut 1, then ~7 spike resolutions each
...
**Unresolved.** Applying "named inside `## Open questions`" to `profac-rbe` blocks REQ-001, 008, 009,
...
Messaging section as `Amlink.OperatingCompanyUpdated`, but flagged in case". REQ-011 is four of the six
...
**Known risk.** 50 rather than `/smell`'s 20 is justified: on `profac-rbe` the tree lands near 36
