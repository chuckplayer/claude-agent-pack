...
**Revisit trigger:** a Claude Code release note stating that `isolation: "worktree"` bases new worktrees on the current HEAD by default, or that the `worktree.baseRef` default changed from `"fresh"` to `"head"`. Verified absent through 2.1.220 (changelog scanned from 2.1.187 forward; the only worktree fixes in that window concern subagents escaping their worktree and worktrees not being deletable/discoverable). When it arrives, steps 1–3 of the fix become belt-and-braces and this file can move to `resolved` — but keep step 4's `git merge-base --is-ancestor` checks regardless, since a project-level override can still reintroduce `"fresh"`.
