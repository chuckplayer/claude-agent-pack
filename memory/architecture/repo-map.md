...
- `merge-reviewer.md` — final pipeline gate; the only agent that commits. Owns gate 4a (plan bars and the three Deviations tiers), plus gates **2a** (`lint-agents.sh`) and **2b** (the obsidian hook suite), which it runs itself rather than trusting a report. Deliberately never runs `lint-plans.sh` — it is forbidden from putting a plan path into a shell command, so gate 4a uses `Grep`/`Read`.
